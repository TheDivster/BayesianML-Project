...
class MapUtahMortalityVis {
    ...
    constructor(globalApplicationState) {
      ...
      const svgElement = d3.select('#utah-mortality-svg').node();
      ...
      const MARGIN = { left: 80, bottom: 80, top: 12, right: 50 };
            ...
            let geoJSON = this.globalApplicationState.utahData;

			const ref = this; // used to access the context of the class

			// Bind data and create one path per GeoJSON feature
			d3.select("#utah-mortality-state").selectAll("path")
    ...
    for (const row of geoJSON.features) {
     ...
     for (const row of globalApplicationState.utahTabData) {
      // d3.csv will read the values as strings; we need to convert them to floats
      dataLookup[row.County] = parseFloat(row.Deaths);
  }
    ...
    d3.select("#utah-mortality-state").selectAll("path")
        ...
        .style("fill", function (d) {
            return color(dataLookup[d.properties.name + " "]);
				...
				d3.select("#utah-mortality-title")
				.append("text")
				...
				.text("Child Mortality by Utah county");

				// Create a linear gradient definition
				const gradient = d3.select("#utah-mortality-svg").append("defs")
				.append("linearGradient")
				.attr("id", "legendGradient-mortality")
				.attr("x1", "0%")
				.attr("y1", "0%")
				.attr("x2", "100%")
				.attr("y2", "0%");
				...
				d3.select("#utah-mortality-svg").append("rect")
				.attr("x", width - 60)
				.attr("y", height + 30)
				.attr("width", 155)
				.attr("height", 20)
				.attr("fill", "url(#legendGradient-mortality)");
		
				d3.select("#utah-mortality-svg").append("text")
				.attr("x", width - 60)
				.attr("y", height + 20)
				.text("0");
