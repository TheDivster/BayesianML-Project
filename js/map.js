...
class MapVis {
    ...
    constructor(globalApplicationState) {
      ...
      const MARGIN = { left: 80, bottom: 80, top: 12, right: 50 };
			...
			.join("path")
			.attr("d", path)
			.attr("class", "state") // Replace with your own logic
			...
			let dataLookup = {};
     for (const row of globalApplicationState.blsData) {
      // d3.csv will read the values as strings; we need to convert them to floats
      dataLookup[row.State] = parseFloat(row.Ratio);
  }
    ...
    d3.select("#states").selectAll("path")
        ...
        .style("fill", function (d) {
            return color(dataLookup[d.properties.name]);
				...
				.text("Ratio of people per hospital employee by State");

				// Create a linear gradient definition
				const gradient = d3.select("#map").append("defs")
